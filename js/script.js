document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderDonatePage(donateData) {
    ...
    // 渲染資金運用與效益 (funds)
    const fundsData = donateData.funds;
    if (fundsData) {
      const fundsTitle = document.getElementById("funds-title");
      const fundsSubtitle = document.getElementById("funds-subtitle");
      if (fundsTitle) fundsTitle.textContent = fundsData.title || "";
      if (fundsSubtitle) fundsSubtitle.textContent = fundsData.subtitle || "";

      const allocationContainer = document.getElementById("funds-allocation-list");
      if (allocationContainer && fundsData.allocations) {
        allocationContainer.innerHTML = fundsData.allocations.map(item => `
          <div class="allocation-item">
            ...
            <div class="allocation-progress-bar">
              <div class="allocation-progress-fill" style="width: ${item.percent}; background-color: ${item.color};"></div>
            </div>
            <p class="allocation-item-note">${item.note}</p>
          </div>
        `).join("");
      }

      const impactContainer = document.getElementById("funds-impact-grid");
      if (impactContainer && fundsData.impacts) {
        impactContainer.innerHTML = fundsData.impacts.map(impact => `
          <div class="impact-card">
            <div class="impact-icon-wrapper">
              <span class="material-symbols-outlined">${impact.icon}</span>
            </div>
            <div class="impact-card-amount">${impact.amount}</div>
            <p class="impact-card-text">${impact.desc}</p>
          </div>
        `).join("");
      }
    }

    // 渲染捐款管道（改為 Google Font Icon 渲染模式）
    ...
    if (methodsList && donateData.methods) {
      methodsList.innerHTML = donateData.methods.map(method => `
        <div class="donate-page-method-item">
          <div class="donate-page-method-icon">
